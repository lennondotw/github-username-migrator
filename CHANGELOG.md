...
- Fix migration error caused by incorrect `migrate` function call signature

## [0.4.0] - 2026-02-03

### Changed

- Inject version and git info at build time via `--define` for accurate build metadata
- Centralize build logic in `scripts/build.sh`
- Dev mode shows `(dev)`, builds show git hash or `(hash, dirty)`

## [0.3.0] - 2026-02-03

### Added

- Scrollable repository list with ↑↓ keys and J/K for page navigation
- Completed steps displayed as static history (usernames, scan results)

### Changed

- Improved URL arrow alignment in repository list
- Better visual feedback during workflow progression
